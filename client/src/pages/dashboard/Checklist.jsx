...
const Checklist = () => {
  ...
  useEffect(() => {
    if (selectedTrip) {
      const saved = localStorage.getItem(`checklist_${selectedTrip._id}`);
      if (saved) {
        try { setCategories(JSON.parse(saved)); } catch { initDefaults(); }
      } else {
        initDefaults();
      }
    }
  ...
  const resetAll = () => {
    setCategories(cs => cs.map(c => ({ ...c, items: c.items.map(i => ({ ...i, packed: false })) })));
  };
  ...
  const handleAdd = () => {
    ...
    setAddItem(''); setAddCat(''); setShowAdd(false);
  };
  ...
  const shareChecklist = () => {
    ...
    categories.forEach(cat => {
      lines.push(`\n${cat.name}`);
      cat.items.forEach(item => lines.push(`  ${item.packed ? '[x]' : '[ ]'} ${item.name}`));
    });
  ...
  if (!loading && trips.length === 0) return (
    <div className="animate-fade-in max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
        ...
        <Link to="/dashboard/trips/new" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-primary-600 to-accent-600 text-white rounded-xl font-semibold text-sm hover:opacity-90 transition-opacity">
    ...
    <div className="animate-fade-in max-w-5xl mx-auto pb-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          {selectedTrip && <p className="text-slate-500 mt-1 text-sm">Trip: <span className="font-semibold text-slate-700">{selectedTrip.title}</span></p>}
        </div>
        <div className="flex gap-2">
          <button onClick={resetAll} className="flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">
            <RotateCcw className="w-4 h-4" /> Reset all
          </button>
          <button onClick={shareChecklist} className="flex items-center gap-1.5 px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 transition-colors">
      ...
      {trips.length > 1 && (
        <div className="mb-5">
          <select value={selectedTrip?._id || ''} onChange={e => setSelectedTrip(trips.find(t => t._id === e.target.value))}
            className="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:border-primary-400 outline-none">
          ...
          {categories.map((cat, ci) => {
            ...
            return (
              <div key={ci} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                <button onClick={() => setCollapsed(prev => ({ ...prev, [ci]: isOpen }))}
                  ...
                  <div className="px-5 pb-4 space-y-1 border-t border-slate-50">
                    {cat.items.length === 0
                      ...
                      : cat.items.map((item, ii) => (
                        <div key={ii} className="flex items-center gap-3 py-2 rounded-xl px-1 hover:bg-slate-50 group transition-colors">
                          <button onClick={() => togglePacked(ci, ii)}
                            className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center transition-all ${item.packed ? 'bg-primary-600 border-primary-600' : 'border-slate-300 hover:border-primary-400'}`}>
                            {item.packed && <svg className="w-3 h-3 text-white" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                          </button>
                          <span className={`flex-1 text-sm transition-colors ${item.packed ? 'line-through text-slate-400' : 'text-slate-700'}`}>{item.name}</span>
                          <button onClick={() => deleteItem(ci, ii)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-50 rounded-lg transition-all">
        ...
        <div className="space-y-4">
            ...
            <div className="relative w-24 h-24 mx-auto mb-4">
              <svg className="w-24 h-24 -rotate-90" viewBox="0 0 96 96">
                ...
                <circle cx="48" cy="48" r="40" fill="none" stroke="#2563eb" strokeWidth="8"
                  strokeDasharray="251" strokeDashoffset={251 - (251 * pct / 100)} strokeLinecap="round" />
              </svg>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
            ...
            <div className="space-y-2.5">
              {categories.map((cat, ci) => {
                const catPct = cat.items.length > 0 ? Math.round((cat.items.filter(i => i.packed).length / cat.items.length) * 100) : 0;
                return (
                  <div key={ci}>
                    ...
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-500 ${cat.color}`} style={{ width: `${catPct}%` }} />
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-emerald-50 rounded-2xl border border-emerald-100 p-4">
            ...
            <p className="text-xs text-emerald-600 leading-relaxed">Review your checklist at least a day before your trip!</p>
          </div>
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl animate-fade-in" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-5 border-b border-slate-100">
                ...
                <button onClick={() => setShowAdd(false)} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors">
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <select value={addCat} onChange={e => setAddCat(e.target.value)}
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:border-primary-400 outline-none">
              ...
              <div className="flex justify-end gap-3 p-5 border-t border-slate-100">
                <button onClick={() => setShowAdd(false)} className="px-5 py-2.5 text-slate-600 text-sm font-medium hover:bg-slate-50 rounded-xl transition-colors">Cancel</button>
                <button onClick={handleAdd} disabled={!addItem.trim()} className="px-6 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Add Item</button>
              </div>
            </div>
          </div>
        </>
